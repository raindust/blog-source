---
title: "我的免费密码管理方案"
date: 2019-08-17T10:22:12+08:00
lastmod: 2019-08-17T10:22:12+08:00
keywords: ["Mac OS", "encryption"]
description: ""
tags: ["Mac OS","Encryptions"]
categories: ["Mac OS"]
---
...
不过这种作法比简单密码安全不到哪里去，如果你注册的某一个网站存在安全漏洞，就有被[拖库](https://baike.baidu.com/item/%E6%8B%96%E5%BA%93/4633384?fr=aladdin)的风险。那么黑客只要拿着从这个网站上你的用户名和密码去其他的网站上去尝试，如果你恰好也注册过黑客就可以轻松入侵，这种攻击方式叫做[撞库](https://baike.baidu.com/item/%E6%92%9E%E5%BA%93)。你可能觉得这种攻击可以通过每个网站的用户名来化解，关于如何让每个用户名不一样你可能想到了和下文中[固定模式密码](#固定模式密码)相同的方式：使用特殊的规则。然而这样也是不安全的，黑客不见得总会拿着完全相同的用户名和密码去另外一个网站上去尝试，他们手中持有的密码库和完全穷举所有的数字、字母及符号的组合比起来完全不在一个数量级，他们即便是整个试一遍成本也不高。还有一点就是，用户名是限定范围内公开甚至是完全公开的，对黑客来说获取某个网站的用户名比获取密码轻松太多了。
...
那么有了随机密码就不会被破解吗？随机数也不一定没有规律可言，这些随机数是通过随机数生成器生成的，而随机数生成器也不见得生成的是真正没有规律的随机数，而是通过一个种子（seed）产生的伪随机数。关于真伪随机数相关概念就不在这里展开讲了，这里有一篇不错的[科普文章](https://www.jianshu.com/p/489ce4cb7628)感兴趣的同学可以点过去看。不过，即便是真随机密码，通过穷举法暴力破解上也是可行的。
...
目前很多网站本身就有一些个人账户的安全保护策略，有部分策略是需要我们来配合的，如：密码存在有效期，过期后尽量按提示重新设置密码；如果支持[2FA](https://baike.baidu.com/item/2FA/14695073?fr=aladdin)尽量设置。这些安全策略对我们的账户安全很有帮助。
...
最近两年我都是用苹果的笔记本和手机，在苹果生态中最有名的当属[1password](https://1password.com/)了，不过目前已经改成按月付费了价格为每个月5$。像我这样没有赶在买断制之前上车的人觉得目前的价格还是有点贵，接下来分享一下我的免费方案吧。

## Mac OS

在Mac上我使用一款Github上的开源软件：[MacPass](https://github.com/MacPass/MacPass)

![MacPass软件界面截图](/images/2019/mac-password-storage/macpass.png)

从上面的截图可看出，MacPass可以分目录组织，存储项不仅包括用户名、密码，还可以存储Url、文本、甚至文件，这样已经足够我们日常使用啦。如果嫌麻烦不愿意自己折腾编译的话，可以到[官网](https://macpassapp.org/)下载。

如果你和我一样也是Alfred的重度用户，还可以去下载一个和MacPass相关的[Workflow](https://github.com/samueldeng/alfred-keepass)，这样就可以在Alfred里输入关键词"kp"来搜索自己所有的用户名及密码了。下图是针对上面新建的用户名密码搜索的截图：

![Alfred搜索截图](/images/2019/mac-password-storage/alfred-search.png)

需要注意的是按照这个Workflow需要额外再按照一个[MacPass的插件](https://github.com/MacPass/MacPassHTTP)，这个插件可以让Alfred和MacPass通过HTTP协议通信，通信安全通过[对称加密](https://baike.baidu.com/item/%E5%AF%B9%E7%A7%B0%E5%8A%A0%E5%AF%86)算法保证。另外，我们通过MacPass拷贝密码之后会有一个过期时间（默认30秒），过期之后会自动从我们电脑的剪切板中删除，但如果使用Alfred这种工具会有记录剪切板历史的，可以根据自己的情况尽量设置一个较短的剪切板存储历史的时间。

有人可能觉得加密的开源软件意味着如何加密的逻辑也是公开的，这样会不安全。然而事实上并非如此，Bruce Schneier在[《应用密码学》](https://book.douban.com/subject/1088180/)一书中提到过，通过保密加密算法以期望达到保密的方式都是不安全的，而目前公开加密标准能够保证足够安全了，所以使用这些公开加密标准的开源软件也是足够安全的。

MacPass会将所有的密码存储在一个文件中，文件格式和[KeePass](https://keepass.info/)是通用的，这也就意味着这个文件也可以在Window系统上使用。我自己把这个文件放在icloud里，然后就可以在IOS系统上共享密码啦。

## IOS

在AppStore里有不少免费的KeePass客户端，我这里也使用了一款Github上的开源软件：[StrongBox](https://github.com/strongbox-password-safe/Strongbox)

![StrongBox的整体界面](/images/2019/mac-password-storage/strongbox-dashboard.png) ![进入新建的加密项](/images/2019/mac-password-storage/strongbox-test.png)
...
不过形如MacPass这种需要主密码的软件，主要采用了对称加密算法对存储于其中的用户名和密码进行加密，如果能够妥善保管好主密码保证不被窃取就已经足够安全了，毕竟绝对安全的[一次性密码本](https://baike.baidu.com/item/%E4%B8%80%E6%AC%A1%E6%80%A7%E5%AF%86%E7%A0%81%E6%9C%AC/8449396?fr=aladdin)成本太高了。
